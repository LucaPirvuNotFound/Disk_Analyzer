...
void handle_server_response(int fd, CommandType type) {
    ...
    if (read(fd, &res, sizeof(res)) <= 0) return;
    ...
    if (type == CMD_ADD) {
        ...
        // Daca Daemon-ul a trimis deja primul nod in primul 'read', il procesam, apoi intram in loop
        ...
        while (!finished) {
            ...
            for (int i = 0; i < num_hashes; i++) printf("#");
            printf("\n");
            ...
            if (res.is_last) {
                ...
                // Citim urmatorul nod
                if (read(fd, &res, sizeof(res)) <= 0) break;
            }
        }
    }
    else {
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "a:p:S:R:r:i:lP:", long_options, NULL)) != -1) {
        switch (opt) {
            case 'a':
                req.type = CMD_ADD;
